...
| Code | Governs | Default State |
|------|---------|-------------|
| Eurocode 0 | Basis of structural design | ✅ Pre-selected (every project) |
| Eurocode 1 (Parts 1–7) | Actions on structures including wind | ✅ Pre-selected (every project) |
| Eurocode 2 (Parts 1–3) | Design of concrete structures | ✅ Pre-selected (every project) |
| Eurocode 3 (Parts 1–12) | Design of steel structures | ✅ Pre-selected (every project) |
| Singapore National Annex | SG-specific parameters (terrain, wind, etc.) | ✅ Pre-selected (every project) |
| SS 602:2014 | Noise control on construction sites | ☐ Optional (acoustic requirements) |

> **Source:** Confirmed from PE calculation report (Lawson Chung, Civil PE 5703). All four Eurocodes plus SG NA are used on every standard temporary barrier project. SS EN designations (SS EN 1990, SS EN 1991-1-4 etc.) are the Singapore-adopted versions of these Eurocodes — they are the same codes.
...
### What to Build in the First Iteration

The first iteration delivers a working frontend shell with Steps 1 and 2 fully functional, and Steps 3–6 as navigable scaffolds. No real calculation logic. No backend calls in Step 1 and 2 beyond file upload.
...
- Barrier digitisation: polyline drawing on canvas using Fabric.js (start/stop/undo/clear)
- Segment table auto-generated from drawn polyline using calibrated scale
- Segment table columns: ID (A, B, C...), Length (m), Tag (dropdown: Type 1 / Type 2 / Type 3)
...
interface ProjectContext {
  ...
  site_data: {
    ...
    alignment_points: { x: number; y: number }[]
    segment_table: { id: string; length_m: number; tag: string }[]
